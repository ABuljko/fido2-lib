...
class MdsEntry {
	...
	constructor(mdsEntry, tocEntry) {
		...
		function matcherProtToArr(mp) {
			...
			if (mp & 0xFFF8) throw new Error("unknown matcher protection flags: " + (mp & 0xFFF8));
			return ret;
		}
		...
		function uvDetailsToSet(uvList) {
			...
			uvList.forEach((uv) => {
				...
				const d = uv.map((desc) => {
					...
					newDesc.userVerification = desc.userVerificationMethod
						? (desc.userVerificationMethod.match(/(\w+)_internal/) || [ "none", "none" ])[1]
						: uvToArr(desc.userVerification);
