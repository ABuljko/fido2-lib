...
/**
 * Maps a pkijs-parsed EC `namedCurve` to a webcrypto curve name.
 * Current pkijs resolves it to the curve name, older releases left the raw OID.
 * used by {@link fromPem}
 *
 * @private
 */
const ecNamedCurves = {
	...
	async fromPem(pem, hashName) {
		...
		if (keyInfo.algorithm.algorithmId === "1.2.840.10045.2.1") {
			...
			algorithm.namedCurve =
				(parsedKey && ecNamedCurves[parsedKey.namedCurve]) || "P-256";
