...
class PublicKey {
	...
	async keyForAlgorithm(alg) {
		...
		try {
			const spki = await tools.webcrypto.subtle.exportKey("spki", key);
			return await tools.webcrypto.subtle.importKey("spki", spki, alg, true, ["verify"]);
		} catch (_e) {
			// Node.js 24+ disables SHA-1, verifySignature falls back to native crypto
			return key;
		}
	}
