...
const validUserVerifications = new Set([
	"required",
	"preferred",
	"discouraged",
]);
...
class Fido2Lib {
	 ...
	 * If "required", the authenticator must have performed user verification. "preferred" and "discouraged" leave the UV flag optional, including for the "first" factor.
...
// own property only, so a polluted Object.prototype can't relax the UV check
function ownUserVerification(expected) {
	return Object.hasOwn(expected, "userVerification")
...
function factorToFlags(expectedFactor, flags, userVerification) {
	...
	if (
		...
		!validUserVerifications.has(userVerification)
	) {
		throw new TypeError(
	...
	const uvUnspecified = userVerification === undefined;

	switch (expectedFactor) {
		case "first":
			flags.push("UP");
			if (uvRequired || uvUnspecified) {
				flags.push("UV");
			}
			break;
		case "second":
