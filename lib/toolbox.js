...
// Size in bytes of each of the two components of a raw ECDSA signature
const ecComponentLengths = {
	"P-256": 32,
	"P-384": 48,
	"P-521": 66,
};
...
function derToRaw(signature, componentLength) {
	...
	// skip the SEQUENCE tag and its length, which is long form for P-521
	let pos = 2;
	if (signature[1] & 0x80) {
		pos += signature[1] & 0x7f;
	}
...
async function verifySignature(publicKey, expectedSignature, data, hashName) {
	...
	try {
		...
		if (alg.name === "ECDSA") {
			const componentLength = ecComponentLengths[alg.namedCurve];
			if (!componentLength) {
				throw new Error("verifySignature: unsupported ECDSA curve " + alg.namedCurve);
			}
